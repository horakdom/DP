...
struct D3Q27_BC_All
{
	...
	CUDA_HOSTDEV static bool BC(DATA &SD, LBM_KS &KS, map_t mapgi, idx xm, idx x, idx xp, idx ym, idx y, idx yp, idx zm, idx z, idx zp)
	{
		switch (mapgi)
		{
		...
		case GEO_OUTFLOW_LEFT:
			STREAMING::streaming(SD,KS,xp,x,xp,ym,y,yp,zm,z,zp);
			COLL::computeDensityAndVelocity(KS);
			KS.rho=no1;
			COLL::collision(KS);
			break;
...
//			STREAMING::streaming(SD,KS,xm,x,xp,ym,y,yp,zm,z,zp);
		...
		case GEO_INFLOW_TOP: // PE GEO_WALL_W_VEL
			STREAMING::streaming(SD,KS,xm,x,xp,ym,y,y,zm,z,zp);
			SD.inflow(KS,x,y,z);
			COLL::collision(KS);
