...
struct D3Q27_CUM_WELL : D3Q27_COMMON_WELL< TRAITS, LBM_EQ >
{
	...
	CUDA_HOSTDEV static void collision(LBM_KS &KS)
	{
		...
		//Eq 13
		const dreal k_100 = (k_p00 - k_m00) - KS.vx * (  k_000 + K_000 );
		const dreal k_101 = (k_p01 - k_m01) - KS.vx * (  k_001 + K_001 );
		const dreal k_102 = (k_p02 - k_m02) - KS.vx * (  k_002 + K_002 );
		const dreal k_110 = (k_p10 - k_m10) - KS.vx * (  k_010 + K_010 );
		const dreal k_111 = (k_p11 - k_m11) - KS.vx * (  k_011 + K_011 );
		const dreal k_112 = (k_p12 - k_m12) - KS.vx * (  k_012 + K_012 );
		const dreal k_120 = (k_p20 - k_m20) - KS.vx * (  k_020 + K_020 );
		const dreal k_121 = (k_p21 - k_m21) - KS.vx * (  k_021 + K_021 );
		const dreal k_122 = (k_p22 - k_m22) - KS.vx * (  k_022 + K_022 );

		//Eq 14
		const dreal k_200 = (k_p00 + k_m00) - no2*KS.vx* (k_p00 - k_m00) + KS.vx*KS.vx * (  k_000 + K_000 );
		const dreal k_201 = (k_p01 + k_m01) - no2*KS.vx* (k_p01 - k_m01) + KS.vx*KS.vx * (  k_001 + K_001 );
		const dreal k_202 = (k_p02 + k_m02) - no2*KS.vx* (k_p02 - k_m02) + KS.vx*KS.vx * (  k_002 + K_002 );
		const dreal k_210 = (k_p10 + k_m10) - no2*KS.vx* (k_p10 - k_m10) + KS.vx*KS.vx * (  k_010 + K_010 );
		const dreal k_211 = (k_p11 + k_m11) - no2*KS.vx* (k_p11 - k_m11) + KS.vx*KS.vx * (  k_011 + K_011 );
		const dreal k_212 = (k_p12 + k_m12) - no2*KS.vx* (k_p12 - k_m12) + KS.vx*KS.vx * (  k_012 + K_012 );
		const dreal k_220 = (k_p20 + k_m20) - no2*KS.vx* (k_p20 - k_m20) + KS.vx*KS.vx * (  k_020 + K_020 );
		const dreal k_221 = (k_p21 + k_m21) - no2*KS.vx* (k_p21 - k_m21) + KS.vx*KS.vx * (  k_021 + K_021 );
		const dreal k_222 = (k_p22 + k_m22) - no2*KS.vx* (k_p22 - k_m22) + KS.vx*KS.vx * (  k_022 + K_022 );

		//Eq 20
		const dreal C_211 = k_211 - (( k_200 + n1o3)*k_011 + no2*k_101*k_110)/KS.rho;
		const dreal C_121 = k_121 - (( k_020 + n1o3)*k_101 + no2*k_110*k_011)/KS.rho;
		const dreal C_112 = k_112 - (( k_002 + n1o3)*k_110 + no2*k_011*k_101)/KS.rho;

		//Eq 21
		const dreal C_220 = k_220- ((( k_020*k_200 + no2*k_110*k_110)+ (k_020 + k_200)*n1o3)/KS.rho - k_000/KS.rho*n1o9);
		const dreal C_022 = k_022- ((( k_002*k_020 + no2*k_011*k_011)+ (k_002 + k_020)*n1o3)/KS.rho - k_000/KS.rho*n1o9);
		const dreal C_202 = k_202- ((( k_200*k_002 + no2*k_101*k_101)+ (k_200 + k_002)*n1o3)/KS.rho - k_000/KS.rho*n1o9);

		//Eq 22
		const dreal C_122 = k_122- (( k_020*k_102 + k_002*k_120 + no4*k_011*k_111 + no2*( k_110*k_012 + k_101*k_021)) + (k_102 + k_120)*n1o3)/KS.rho;
		const dreal C_212 = k_212- (( k_002*k_210 + k_200*k_012 + no4*k_101*k_111 + no2*( k_011*k_201 + k_110*k_102)) + (k_210 + k_012)*n1o3)/KS.rho;
		const dreal C_221 = k_221- (( k_200*k_021 + k_020*k_201 + no4*k_110*k_111 + no2*( k_101*k_120 + k_011*k_210)) + (k_021 + k_201)*n1o3)/KS.rho;
		//Eq 23
		const dreal C_222 = k_222 - ( no4*k_111*k_111 + k_200*k_022 + k_020*k_202 + k_002*k_220 + no4*(k_011*k_211 + k_101*k_121 + k_110*k_112) + no2*(k_120*k_102 + k_210*k_012 + k_201*k_021) )/KS.rho +(no16*k_110*k_101*k_011 + no4*(k_101*k_101*k_020 + k_011*k_011*k_200 + k_110*k_110*k_002)+no2*k_200*k_020*k_002)/KS.rho/KS.rho - ( no3*(k_022+k_202+k_220)+(k_200+k_020+k_002) )*n1o9/KS.rho +no2* ( no2*(k_101*k_101 + k_011*k_011 + k_110*k_110) + (k_002*k_020 + k_002*k_200 + k_020*k_200) + (k_002+k_020+k_200)*n1o3 )*n1o3/KS.rho/KS.rho + (k_000*k_000 - k_000)*n1o27/KS.rho/KS.rho;
		...
		// gen2.php BEGIN
		//Eq. 53
		const dreal ks_211 = Cs_211+ ((ks_200 + n1o3)*ks_011 + no2*ks_101*ks_110)/KS.rho;
		const dreal ks_121 = Cs_121+ ((ks_020 + n1o3)*ks_101 + no2*ks_110*ks_011)/KS.rho;
		const dreal ks_112 = Cs_112+ ((ks_002 + n1o3)*ks_110 + no2*ks_011*ks_101)/KS.rho;

		//Eq. 54
		const dreal ks_220 = Cs_220+ (((ks_020*ks_200 + no2*ks_110*ks_110)+ (ks_020+ks_200)*n1o3)/KS.rho- k_000/KS.rho*n1o9);
		const dreal ks_022 = Cs_022+ (((ks_002*ks_020 + no2*ks_011*ks_011)+ (ks_002+ks_020)*n1o3)/KS.rho- k_000/KS.rho*n1o9);
		const dreal ks_202 = Cs_202+ (((ks_200*ks_002 + no2*ks_101*ks_101)+ (ks_200+ks_002)*n1o3)/KS.rho- k_000/KS.rho*n1o9);

		//Eq. 55
		const dreal ks_122 = Cs_122+ ((ks_020*ks_102 + ks_002*ks_120+ no4*ks_011*ks_111+ no2*(ks_110*ks_012+ks_101*ks_021))+ (ks_102+ks_120)*n1o3)/KS.rho;
		const dreal ks_212 = Cs_212+ ((ks_002*ks_210 + ks_200*ks_012+ no4*ks_101*ks_111+ no2*(ks_011*ks_201+ks_110*ks_102))+ (ks_210+ks_012)*n1o3)/KS.rho;
		const dreal ks_221 = Cs_221+ ((ks_200*ks_021 + ks_020*ks_201+ no4*ks_110*ks_111+ no2*(ks_101*ks_120+ks_011*ks_210))+ (ks_021+ks_201)*n1o3)/KS.rho;
		// gen2.php END
		// Eq. 56
		const dreal ks_222 = Cs_222 + (no4*ks_111*ks_111 + ks_200*ks_022 + ks_020*ks_202 + ks_002*ks_220 + no4*(ks_011*ks_211 + ks_101*ks_121 + ks_110*ks_112)
					...
					+ (no3*(ks_022+ks_202+ks_220) + (ks_200+ks_020+ks_002))*n1o9/KS.rho
					-n2o3*(no2*(ks_101*ks_101+ks_011*ks_011+ks_110*ks_110) + (ks_002*ks_020+ks_002*ks_200+ks_020*ks_200) + (ks_002+ks_020+ks_200)*n1o3)/KS.rho/KS.rho
					-(k_000*k_000 - k_000)*n1o27/KS.rho/KS.rho;
