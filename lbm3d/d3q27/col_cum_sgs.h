...
// tak kdyz tohle zadelas pred kolizi do Cumulantu tak bys tammel mit LES s SGS=Smagorinsky-Lilly ];)
// #define C 0.18 //Smagorinsky constant (0.1-0.2 ];))) )
//    const dreal Q = no2*((C_200-n1o3*k_000)*(C_200-n1o3*k_000)+(C_020-n1o3*k_000)*(C_020-n1o3*k_000)+(C_002-n1o3*k_000)*(C_002-n1o3*k_000)+no2*(C_110*C_110+C_101*C_101+C_011*C_011));
//    const dreal tau = no3*KS.lbmViscosity+n1o2;
//    const dreal omega1 = no2/(sqrt(tau*tau+no2*C*C*no3*no3*sqrt(Q)/k_000)+tau); //Smagorinsky SGS LES
//jo jeste je mozny ze omega ktery kontroluje bulk viscosity musi bejt stejny jak omega pro shear viscosity aby sedela aproximace strain rate tenzoru, ale to musim jeste prozkoumat, zatim by to mohlo taghle slapat...

template <
	typename TRAITS,
	typename LBM_EQ=D3Q27_EQ<TRAITS>
>
struct D3Q27_CUM_SGS : D3Q27_COMMON< TRAITS, LBM_EQ >
{
	...
	CUDA_HOSTDEV static void collision(LBM_KS &KS)
	{
		...
		//
// #define C 0.18
		const dreal C=0.18; //Smagorinsky constant (0.1-0.2 ];))) )
		const dreal Q = no2*((C_200-n1o3*k_000)*(C_200-n1o3*k_000)+(C_020-n1o3*k_000)*(C_020-n1o3*k_000)+(C_002-n1o3*k_000)*(C_002-n1o3*k_000)+no2*(C_110*C_110+C_101*C_101+C_011*C_011));
		const dreal tau = no3*KS.lbmViscosity+n1o2;
		const dreal omega1 = no2/(sqrt(tau*tau+no2*C*C*no3*no3*sqrt(Q)/k_000)+tau); //Smagorinsky SGS LES
		const dreal omega2 = omega1; // FIXME?
